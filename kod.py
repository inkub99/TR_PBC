...
selected_miesiace = [353,354,355,356,357,358,359,360,361,362,363,364]
    ...
    for j in pisma_lista:
        for k in wskaźniki_lista:
            ...
            if k != 'Total Reach 360°':
                ...
                wyniki.loc[j, k] = max(int(wyniki.loc[j, 'Druk i E-wydania']), (1 - float(df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].iloc[0]/100)) * wyniki.loc[j, 'Druk i E-wydania'] + wyniki.loc[j, 'www']) if not df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].empty else wyniki.loc[j, 'Druk i E-wydania'] + wyniki.loc[j, 'www']

                wyniki_cal.loc[j, k] = max(int(wyniki_cal.loc[j, 'Druk i E-wydania']), (1 - float(df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].iloc[0]/100) ) * wyniki_cal.loc[j, 'Druk i E-wydania'] + wyniki_cal.loc[j, 'www']) if not df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].empty else wyniki_cal.loc[j, 'Druk i E-wydania'] + wyniki.loc[j, 'www']
...
st.markdown("""<div style="font-size:12px">Fale: 1/2024-12/2024</div>""", unsafe_allow_html=True)
