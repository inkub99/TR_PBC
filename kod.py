    ...
    for j in pisma_lista:
        for k in wskaźniki_lista:
            ...
            if k != 'Total Reach 360°':
                ...
                wyniki.loc[j, k] = max(int(wyniki.loc[j, 'Druk i E-wydania']), (1 - float(df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'])) * wyniki.loc[j, 'Druk i E-wydania'] + wyniki.loc[j, 'www']) if not df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].empty else wyniki.loc[j, 'Druk i E-wydania'] 

                wyniki_cal.loc[j, k] = max(int(wyniki_cal.loc[j, 'Druk i E-wydania']), (1 - float(df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK']) ) * wyniki_cal.loc[j, 'Druk i E-wydania'] + wyniki_cal.loc[j, 'www']) if not df.loc[(df['tytuł'] == j) & (df['WSKAŹNIK'] == 'współczytelnictwo'), 'WYNIK'].empty else wyniki_cal.loc[j, 'Druk i E-wydania'] 
